<template>
  <transition name="slide">
    <div class="line-match" ref="lineMatch" :style="getDayBackgroundSty">
      ...
      <div class="btn-group">
        <span @click.stop="btnNextCourse" v-show="end && score === 20" class="btn" :style="getDayBackgroundMenuSty">{{getPass}}</span>
  ...
  export default {
    ...
    methods: {
      ...
      btnNextCourse() {
        ...
        this._getWords()
        this.$router.push({
          path: `/courses/${this.currentCourse}/${challengeMode.lineMatch.eName}`
        })
